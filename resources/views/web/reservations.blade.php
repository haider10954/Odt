    ...
    <table class="table theme-table table-responsive">
        ...
        <tbody>
            ...
            <tr>
                <td>{{ $loop->index + 1 }}</td>
                <td>
                    <div class="d-flex align-items-center justify-content-left ms-5">
                ...
                <td>
                    ...
                    @endphp
                    <a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#ticket_info" class="btn btn-dark text-white btn-table-theme ticket_info_btn" data-club-name="{{ $item->ticket->club_name }}" data-subject="{{ $item->ticket->subject }}" data-sub-description="{{ $item->ticket->sub_description }}" data-gatherings="{{ $item->ticket->gatherings.' 명' }}" data-meet-up-date="{{\Carbon\Carbon::parse($item->ticket->meet_up_date)->format('d M, Y')}}" data-meetups="{{ $item->ticket->meetups.' 회' }}" data-date-last-meeting="{{\Carbon\Carbon::parse($item->ticket->date_last_meeting)->format('d M, Y')}}" data-description="{{ $item->ticket->description }}" data-tag-1="{{ $tag1_string }}" data-tag-2="{{ $tag2_string }}">{{ __('translation.TICKET INFO') }}</a>
                </td>
            </tr>
            @endforeach
            @else
                                ...
                                <div class="right-content mx-2">
                                    ...
                                    <div class="ticket_item_content-footer">
                                        <div class="my-1">
                                            ...
                                            <button type="button" data-id="1" data-bs-toggle="modal" data-bs-target="#reservationModal" class="btn btn-warning btn-theme-sm w-100 applyBtn" disabled="">{{__('translation.Reserved')}}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
